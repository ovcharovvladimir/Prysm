...
import (
	...
	"github.com/ovcharovvladimir/Prysm/sness/observer"
	"github.com/ovcharovvladimir/Prysm/sness/params"
	"github.com/ovcharovvladimir/Prysm/sness/proposer"
	"github.com/ovcharovvladimir/Prysm/sness/simulator"
	"github.com/ovcharovvladimir/Prysm/sness/syncer"
	"github.com/ovcharovvladimir/Prysm/sness/txpool"
	"github.com/ovcharovvladimir/Prysm/sness/utils"
	"github.com/ovcharovvladimir/Prysm/sness/voter"
...
func New(ctx *cli.Context) (*ShardEthereum, error) {
	...
	modeFlag := ctx.GlobalString(utils.ModeFlag.Name)
	if err := shardEthereum.registerTXPool(modeFlag); err != nil {
		return nil, err
	}
	...
	if err := shardEthereum.registerActorService(shardEthereum.shardConfig, modeFlag, shardIDFlag); err != nil {
		return nil, err
	}
...
func (s *ShardEthereum) registerMainchainClient(ctx *cli.Context) error {
	...
	client, err := mainchain.NewVRCClient(endpoint, path, depositFlag, passwordFile)
	if err != nil {
		return fmt.Errorf("could not register vrc client service: %v", err)
	}
...
// Registers the actor according to CLI flags. Either voter/proposer/observer.
func (s *ShardEthereum) registerActorService(config *params.Config, actor string, shardID int) error {
	var shardp2p *p2p.Server
	if err := s.services.FetchService(&shardp2p); err != nil {
		return err
	}
	var client *mainchain.VRCClient
	...
	switch actor {
	case "voter":
		not, err := voter.NewNotary(config, client, shardp2p, shardChainDB)
		if err != nil {
			return fmt.Errorf("could not register voter service: %v", err)
		}
