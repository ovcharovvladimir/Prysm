...
// RequestCollationBody fetches a collation header record submitted to the VRC for
...
// In production, this will be done within a voter service.
func RequestCollationBody(fetcher mainchain.RecordFetcher, shardID *big.Int, period *big.Int) (*pb.CollationBodyRequest, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("could not fetch collation record from VRC: %v", err)
	}
