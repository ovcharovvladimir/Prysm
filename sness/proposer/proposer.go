...
func AddHeader(client mainchain.FullClient, transactor mainchain.ContractTransactor, collation *types.Collation) error {
	...
	tx, err := transactor.VRCTransactor().AddHeader(txOps, collation.Header().ShardID(), collation.Header().Period(), [32]byte(*collation.Header().ChunkRoot()), collation.Header().Sig())
	if err != nil {
		return fmt.Errorf("unable to add header to VRC: %v", err)
	}
...
func createCollation(caller mainchain.ContractCaller, account *accounts.Account, signer mainchain.Signer, shardID *big.Int, period *big.Int, txs []*gethTypes.Transaction) (*types.Collation, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("can't get shard count from vrc: %v", err)
	}
	...
	// check with VRC to see if we can add the header.
...
func checkHeaderAdded(caller mainchain.ContractCaller, shardID *big.Int, period *big.Int) (bool, error) {
	...
	lastPeriod, err := caller.VRCCaller().LastSubmittedCollation(&bind.CallOpts{}, shardID)
	if err != nil {
